...
impl NetworkProperties {
    ...
    pub fn initialize_infection_gillespie(&mut self, network: &NetworkStructureDuration, num_infec: usize, num_dur: usize) {
        ...
        for &i in selected.iter() {
            self.nodal_states[i] = State::Infected;
    ...
    pub fn count_states(&self) -> Vec<usize> {
        ...
        for state in self.nodal_states.iter() {
            match state {
                ...
                State::Exposed1 | State::Exposed2 | State::Exposed3 => result[1] += 1,
    ...
    pub fn count_states_age(&self, network: &NetworkStructure) -> Vec<Vec<usize>> {
        let mut result: Vec<Vec<usize>> = vec![vec![0; 4]; network.ages.iter().max().unwrap().to_owned()+1];
        for (i, state) in self.nodal_states.iter().enumerate() {
            match state {
                &State::Susceptible => result[network.ages[i]][0] += 1,
                State::Exposed1 | State::Exposed2 | State::Exposed3 => result[network.ages[i]][1] += 1,
                &State::Infected => result[network.ages[i]][2] += 1,
                &State::Recovered => result[network.ages[i]][3] += 1
            }
        }
        result
    }
